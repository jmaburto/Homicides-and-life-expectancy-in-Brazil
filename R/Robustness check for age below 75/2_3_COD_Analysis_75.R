...
load('Data/Decomposition_Results_75.RData')
...
Decomposition.results.75 <- Decomposition.results.75[age.group<75]

Cause.contribution <- Decomposition.results.75[year %in% (initial.year+1):final.year
...
pdf(file="R/Robustness check for age below 75/Cause_contrib_Females_2000-15_75.pdf",width=10,height=9,useDingbats = F)
...
pdf(file="R/Robustness check for age below 75/Cause_contrib_Males_2000-15_75.pdf",width=10,height=9,useDingbats = F)
...
e75.change.decomp <- Decomposition.results.75[year %in% (initial.year+1):final.year
                                          ,list(e0.change = sum(contribution)),
                                          by = list(region,state_code,state,sex)]
e75.change.decomp$source <- 'Decomp'

load('Data/Check_LifeExpectancy75_Brazil03Nov2019.RData')

e75.Brazil.states <- e75.Brazil.states[,c(2,4,3,5,6,1)]
names(e75.Brazil.states)[5] <- 'e0'

e0.change.julia <- e75.Brazil.states[,list(e0.change = e0[length(e0)] - e0[1]), by = list(region,state_code,state,sex)]

e0.change.julia$source <- 'Julia'

e0.data <- rbind(e75.change.decomp,e0.change.julia)
...
Change.ex <- ggplot(e75.change.decomp, aes(e0.change, state)) +
  ggtitle(paste0(' change in life expectancy in ',initial.year,'-',final.year))+
  geom_vline(xintercept = 0)+
  #xlim(c(-3,3))+
  geom_point(data = e75.change.decomp, aes(e0.change, state,col=sex, shape=sex),size = 3) +
...
pdf(file="R/Robustness check for age below 75/Change_life_expectancy_2000-15_75.pdf",width=10,height=9,useDingbats = F)
Change.ex
dev.off()

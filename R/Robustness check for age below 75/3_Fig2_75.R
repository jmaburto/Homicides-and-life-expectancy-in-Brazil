...
Cause.contribution1 <- Decomposition.results.75[year %in% (initial.year1+1):final.year1
                                             ,list(Period = '2000-07',
                                                   contribution = sum(contribution)),
                                             by = list(region,state_code,state,sex,cause)]

Cause.contribution2 <- Decomposition.results.75[year %in% (initial.year2+1):final.year2
                                             ,list(Period = '2007-15',
                                                   contribution = sum(contribution)),
                                             by = list(region,state_code,state,sex,cause)]
...
gdata::keep(Cause.contribution,Decomposition.results.75,sure = T)
...
pdf(file="R/Robustness check for age below 75/Figure_2_Homicides_75.pdf",width=10,height=9,useDingbats = F)
Fig2
dev.off()
...
pdf(file="R/Robustness check for age below 75/Figure_3_IHD_75.pdf",width=10,height=9,useDingbats = F)
Fig3
dev.off()
...
pdf(file="R/Robustness check for age below 75/Figure_4_Amenable_75.pdf",width=10,height=9,useDingbats = F)
Fig4
dev.off()

...
e0.change1 <- Decomposition.results.75[year %in% (initial.year1+1):final.year1
                                           ,list(period = '2000-07',
                                                 e0.change = sum(contribution)),
                                           by = list(region,state_code,state,sex)]
e0.change1 <- e0.change1[order(sex,region,state_code)]

e0.change2 <- Decomposition.results.75[year %in% (initial.year2+1):final.year2
                                           ,list(period = '2007-15',
                                                 e0.change = sum(contribution)),
                                           by = list(region,state_code,state,sex)]
...
gdata::keep(Decomposition.results.75,e0.change, sure = T)
...
pdf(file="R/Robustness check for age below 75/Figure_1_75.pdf",width=10,height=9,useDingbats = F)
Fig1
dev.off()
